...
## Magnet and oversized-file support

This version preserves the existing Pyrogram upload path for files up to `TG_MAX_FILE_SIZE` (2,194,304,000 bytes). The repository uses Pyrogram with a bot identity, so the existing approximately 1.95GB upload behavior is retained.

Private messages containing a `magnet:?xt=urn:btih:...` URI are downloaded through `aria2c`. Each completed torrent file is uploaded sequentially. When a downloaded file exceeds `TG_MAX_FILE_SIZE`, it is streamed into large numbered document parts using `UPLOAD_PART_SIZE` (default `1,950,000,000` bytes), for example `Part 1/2` and `Part 2/2`; the original 1.95GB path is not replaced by small 45MiB Bot API parts.

For Railway, deploy from the Dockerfile so that `aria2` is present. Mount a persistent Volume at `/app/DOWNLOADS`. The Volume must have enough free space for the completed source file plus at least one temporary split part and torrent metadata. A redeploy without a Volume can remove active downloads.
