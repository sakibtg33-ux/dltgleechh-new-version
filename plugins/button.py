...
async def youtube_dl_call_back(bot, update):
    ...
    if process.returncode == 0:
        ...
        if file_size > Config.TG_MAX_FILE_SIZE:
            await update.message.edit_caption(
                caption=f"File size {humanbytes(file_size)}; numbered parts এ upload হচ্ছে..."
            )
            await send_file_in_parts(
                update.message,
                download_directory,
                caption=description,
                progress_message="Uploading part...",
            )
            shutil.rmtree(tmp_directory_for_each_user, ignore_errors=True)
            await update.message.edit_caption(caption="Large file parts upload complete.")
            return True
        else:
            await update.message.edit_caption(
            ...
            shutil.rmtree(tmp_directory_for_each_user, ignore_errors=True)
            for temporary_thumb in (locals().get("thumbnail"), locals().get("thumb_image_path")):
                if temporary_thumb and os.path.exists(temporary_thumb):
                    try:
