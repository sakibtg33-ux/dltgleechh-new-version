...
async def cancel_cb(c, m):
    ...
    job_path = m.data.split("+", 1)[1]
    if not os.path.exists(job_path):
        await m.message.edit("This process is already finished or the bot was restarted")
        return
    try:
        if os.path.isdir(job_path):
            ...
            os.remove(job_path)
        await m.message.edit("Download cancelled")
    except OSError:
        await m.message.edit("Unable to cancel this download")
